...
public class FoodViewMapper {
    ...
    public static FoodView toFoodView(Food food) {
        return new FoodView(food.getBrand().getName(), food.getName(), food.getCarbohydrates(), food.getCalories(),
    ...
    public static FoodSummaryView toFoodSummaryView(Food food) {
        return new FoodSummaryView(food.getBrand().getName(), food.getName(), food.getIngestionType().name());
    }
}
